...
<br>
<footer class="footer mt-auto py-3 text-muted ${UI.footer_style_class}" style="${UI.fixed_footer ?'position: fixed; ': ''}left: 0; bottom: 0; width: 100%; color: white; z-index: 9999;${UI.hide_footer ? ' display:none;': ' display:block;'}"> <div class="container" style="width: auto; padding: 0 10px;"> <p class="float-end"> <a href="#">Trở về đầu trang</a> </p> ${UI.credit ? '<p>Redesigned with <svg width="1em" height="1em" viewBox="0 0 16 16" class="bi bi-heart-fill" fill="red" xmlns="http://www.w3.org/2000/svg"> <path fill-rule="evenodd" d="M8 1.314C12.438-3.248 23.534 4.735 8 15-7.534 4.736 3.562-3.248 8 1.314z" /> </svg> by <a href="https://www.npmjs.com/package/@googledrive/index" target="_blank">TheFirstSpeedster</a>, based on Open Source Softwares.</p>' : ''} <p><a href="https://tnhan.dev" target="_blank"> Thien Nhan Drive</a> | <a href="https://www.facebook.com/tnhantl" target="_blank"> Facebook </a> | <a href="https://t.me/tnhantl" target="_blank"> Telegram</a></a></p> </div> </footer>
  `;
	$('body').html(html);
}
...
function nav(path) {
  ...
  <a class="navbar-brand" href="/">Thien Nhan Drive</a>
	...
	try {
		...
		if (is_lastpage_loaded) {
			...
			if (total_files == 0) {
				...
				$('#count').removeClass('d-none').find('.totalsize').text(total_files + " tập tin (thư mục) có kích thước " + total_size);
			} else {
				$('#count').removeClass('d-none').find('.totalsize').text(total_files + " tập tin (thư mục) có kích thước " + total_size);
			}
		}
